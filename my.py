...
async def vector_search_node(state: VectorSearchInput) -> dict:
    ...
    """
    # Initialize kwargs from state or empty dict
    kwargs = state.field_filter or {}
    
    # CRITICAL FIX: Force defaults to prevent 422 Error
    if "search_type" not in kwargs:
        kwargs["search_type"] = "semantic"
    
    # The API requires a re-ranker for semantic/hybrid search. 
    # Defaulting to 'RRF' (Reciprocal Rank Fusion) ensures the payload is valid.
...
def collect_search_results_node(state: SearchState) -> SearchState:
    ...
    # We use title + page + content to identify unique chunks
...
async def search_select_graph(state: SearchState):
    ...
    # 4. Limit candidates (e.g. top 15)
    max_docs = Config.tachyon.max_documents_considered
    docs = docs[:max_docs]

    # 5. Create string representation for LLM
    ...
    # 7. Match strings back to objects
